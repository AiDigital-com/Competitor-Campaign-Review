 ...
 *
 * Single 60s pass. Commercial LPs (coca-cola.com, pepsi.com, drpepper.com)
 * need ~30–45s end-to-end through Firecrawl's anti-bot renderer; the
 * previous defaults (~25s) timed out on ~half the targets, leaving blank
 * cards in V6. Verified via direct Firecrawl probes against the exact URLs
 * that came back empty in production sessions.
 */
  ...
  try {
    const res = await fetch(`${FIRECRAWL_BASE}/scrape`, {
      ...
      body: JSON.stringify({
        ...
        waitFor: 1500,      // settle time after initial load
        timeout: 60000,     // ceiling — covers heavy JS + anti-bot renderer
