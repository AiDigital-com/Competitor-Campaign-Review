...
const [rows] = await bq.query({
  query: `SELECT LOWER(advertiser_domain) as advertiser_domain, 'United States' as country, creative_campaign_name, channel_name, advertiser_master_category, advertiser_second_category, transaction_method, ANY_VALUE(creative_landingpage_url) as landing_page_url, SUM(impressions) as impressions, SUM(spend) as spend, AVG(ctr) as ctr, COUNT(DISTINCT creative_id) as creative_count, COUNT(DISTINCT publisher_domain) as publisher_count, MIN(creative_first_seen_date) as first_seen, MAX(creative_last_seen_date) as last_seen FROM ${T} WHERE ${DF} AND LOWER(SUBSTR(advertiser_domain, 1, 1)) = @letter GROUP BY 1,3,4,5,6,7 ORDER BY impressions DESC`,
  params: { letter },
});
console.log(`[${letter}] BQ returned ${rows.length} rows in ${((Date.now() - t0) / 1000).toFixed(1)}s`);

// Delete existing rows for this letter range
const next = String.fromCharCode(letter.charCodeAt(0) + 1);
await sb.from('ccr_campaign_channel_detail').delete().gte('advertiser_domain', letter).lt('advertiser_domain', next);
console.log(`[${letter}] Truncated existing rows`);

// Batch insert
let inserted = 0;
for (let i = 0; i < rows.length; i += BATCH) {
  const batch = rows.slice(i, i + BATCH).map(ser);
  const { error } = await sb.from('ccr_campaign_channel_detail').insert(batch);
  if (error) {
    console.error(`[${letter}] Insert error at batch ${i}:`, error.message);
    // Print the first row of the failing batch for debugging
    if (i === 0 || inserted === 0) console.error('Sample row:', JSON.stringify(batch[0]));
    break;
  }
